...
function AllTask() {
  ...
  const handleDelete = (id) => {
    ...
    const updatedEmployees = userData.employees.map(emp => {
      if (emp.name === taskToDelete.assignedTo) {
        const newTasks = emp.tasks.filter((_, i) => i !== taskToDelete.originalIndex);
  ...
  const handleSave   = () => {
    ...
    let oldEmpName = taskToEdit.assignedTo;
    let newEmpName = editTask.assignedTo;
    let originalIndex = taskToEdit.originalIndex;

    let newEmployees = [...userData.employees];
    let oldEmpIndex = newEmployees.findIndex(e => e.name === oldEmpName);
    let newEmpIndex = newEmployees.findIndex(e => e.name === newEmpName);

    if (oldEmpIndex !== -1) {
      let originalTask = newEmployees[oldEmpIndex].tasks[originalIndex];
      let updatedTask = {
        ...originalTask,
        ...
        active: editTask.status === "In Progress" || editTask.status === "Completed", 
        failed: false,
      ...
      if (oldEmpName === newEmpName) {
        let newTasks = [...newEmployees[oldEmpIndex].tasks];
        newTasks[originalIndex] = updatedTask;
        newEmployees[oldEmpIndex] = { ...newEmployees[oldEmpIndex], tasks: newTasks };
      } else if (newEmpIndex !== -1) {
        let oldTasks = [...newEmployees[oldEmpIndex].tasks];
        oldTasks.splice(originalIndex, 1);
        newEmployees[oldEmpIndex] = { ...newEmployees[oldEmpIndex], tasks: oldTasks };

        let newTasks = [...newEmployees[newEmpIndex].tasks, updatedTask];
        newEmployees[newEmpIndex] = { ...newEmployees[newEmpIndex], tasks: newTasks };
      }
    }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-950 via-slate-900 to-teal-950 px-8 py-10 font-sans relative overflow-x-hidden">

      {/* Ambient glow blobs */}
      <div className="fixed inset-0 pointer-events-none z-0">
        <div className="absolute top-[15%] left-[10%] w-[400px] h-[400px] bg-emerald-400/5 rounded-full blur-[80px]" />
        <div className="absolute bottom-[20%] right-[10%] w-[350px] h-[350px] bg-blue-400/5 rounded-full blur-[80px]" />
        <div className="absolute top-[60%] left-[40%] w-[300px] h-[300px] bg-purple-400/5 rounded-full blur-[80px]" />
      </div>
      ...
      <div className="relative z-10 max-w-7xl mx-auto">
        ...
        {/* ── Header ── */}
        <div className="mb-9">
          <h1 className="text-[34px] font-extrabold text-slate-100 tracking-tight mb-1">
            All Tasks
          </h1>
          <p className="text-slate-400 text-sm">Organised by category · Click a header to collapse</p>
        </div>
